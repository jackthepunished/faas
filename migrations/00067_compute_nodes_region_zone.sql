...
-- filename: 00067_compute_nodes_region_zone.sql
--
-- 00067_compute_nodes_region_zone.sql — multi-box placement scheduler
...
--
-- The columns are nullable so pre-00067 schedd rows (the seeded
...
comment on column compute_nodes.region is
    'Locality label for the chooser tie-break (pkg/sched/placement.go). Free-form text; nullable so pre-00067 rows accept the schema. The seeded default-local row is backfilled to ''local''. ADR-025.';
...
-- tie-break on name, which is the pre-00067 behaviour.
