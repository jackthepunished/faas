...
// Migration-apply test for 00191_edge_rules.sql (ADR-089 / PR 1 of
...
//
// Seed UUIDs carry the slot number in the last group (`...000191`,
// `...000291`, `...000391`) so a reader scanning the test fixtures
...
func TestMigrations_00191_EdgeRules(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00191.
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00000000-0000-0000-0000-000000000191',
	...
	if err := pool.QueryRow(ctx, `
		select count(*) from edge_rules
		 where app_id = '00000000-0000-0000-0000-000000000291'
	...
	if _, err := pool.Exec(ctx, `
		delete from apps where id = '00000000-0000-0000-0000-000000000291'
	`); err != nil {
