...
// Migration-apply test for 00328_preview_destroy_commented_at.sql
// (Mega-C PR-1 / issue #961 leaf 3).
//
// Pins:
//
//  1. Migration set applies cleanly through 00328 (no goose
//     duplicate-version panic). Slot 00328 was picked as the
//     next free slot past:
//       - 00302 (origin/main, deployments.stage_state — landed
//         via PR #985 / issue #961 stage-state foundation)
//       - 00303 (PR #992 / deployments.actor)
//       - 00304 (PR #984 / reserve_slot fence)
//       - 00305 (PR #984 / deployments.annotation)
...
func TestMigrations_00328_PreviewDestroyCommentedAt(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00306. Pins slot hygiene — if a
	// cross-PR collision sneaks past the precheck (a fence
	// from another PR was re-claimed, etc.) this is the
	// line that catches it.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (regression: 00328 must apply cleanly; check open-PR fences via scripts/ci/check_migration_slots.sh)", err)
	}
	...
	if !exists {
		t.Fatal("apps.preview_destroy_commented_at missing after 00328 (column add failed silently — likely a replay-safety regression)")
	}
	...
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("replay db.MigrateUp: %v (00328 must be replay-safe via ADD COLUMN IF NOT EXISTS)", err)
	}
}
