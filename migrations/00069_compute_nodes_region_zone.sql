...
-- filename: 00069_compute_nodes_region_zone.sql
--
-- 00069_compute_nodes_region_zone.sql — multi-box placement scheduler
-- (ADR-025/028/029, scale-out worktree). Renumbered from 00067 at PR
-- time (see commit 6c0a5b2e) to dodge a slot collision with PR #428
-- which landed migrations/00067_extend_metering_telemetry.sql first.
-- The semantic content is unchanged; only the slot moved.
--
...
--
-- The columns are nullable so pre-00069 schedd rows (the seeded
...
comment on column compute_nodes.region is
    'Locality label for the chooser tie-break (pkg/sched/placement.go). Free-form text; nullable so pre-00069 rows accept the schema. The seeded default-local row is backfilled to ''local''. ADR-025.';
...
-- tie-break on name, which is the pre-00069 behaviour.
