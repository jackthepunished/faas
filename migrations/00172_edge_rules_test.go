...
// Migration-apply test for 00172_edge_rules.sql (ADR-089 / PR 1 of
...
// Pins:
//
//  1. Migration set applies cleanly through 00172.
...
//
// Seed UUIDs carry the slot number in the last group (`...000172`,
// `...000272`, `...000372`) so a reader scanning the test fixtures
...
func TestMigrations_00172_EdgeRules(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00172.
	...
	if err := pool.QueryRow(ctx, `
		select match_path, match_methods, priority, enabled, kind
		  from edge_rules
		 where id = '00000000-0000-0000-0000-000000000472'
	...
	if err := pool.QueryRow(ctx, `
		select count(*) from edge_rules
		 where app_id = '00000000-0000-0000-0000-000000000272'
	...
	if _, err := pool.Exec(ctx, `
		delete from apps where id = '00000000-0000-0000-0000-000000000272'
	`); err != nil {
