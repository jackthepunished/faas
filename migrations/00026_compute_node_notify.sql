...
-- SetComputeNodeActive both write to compute_nodes; gatewayd-internal listens
-- on this channel to drop its per-node *grpc.ClientConn cache entry
-- when the row mutates (overlay IP rotated, node drained). The
-- payload is JSON {"node_id":"<uuid>","active":<bool>}; gatewayd-internal
...
-- gatewayd-internal's existing channels deal in app/app-id semantics — the
-- routing cache and the per-app wake gate. compute_node_changed
-- deals in node-id semantics — the per-node vmmd client cache.
-- Splitting the channels lets gatewayd-internal route the eviction to a
