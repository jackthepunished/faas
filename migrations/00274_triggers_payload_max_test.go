...
// Migration-apply test for 00274_triggers_payload_max.sql (per-trigger
// payload size cap, audit #7 from PR #910).
//
// Pins:
//
//  1. Migration set applies cleanly through 00274 (no goose
...
func TestMigrations_00274_TriggersPayloadMax(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply cleanly.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (PR follow-up failure mode: missing migration slot between 00273 triggers and 00274 payload_max)", err)
	}
