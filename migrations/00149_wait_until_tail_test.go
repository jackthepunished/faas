...
// TestMigrations_00149_WaitUntilTail pins the IDEMPOTENT ALTER shape
// and the column types of the tail-count + tail-seconds columns
// landed by 00149_wait_until_tail.sql (issue #667 / ADR-078).
//
...
func TestMigrations_00149_WaitUntilTail(t *testing.T) {
	...
	for table, cols := range want {
		for col, spec := range cols {
			...
			if err != nil {
				t.Errorf("%s.%s column missing — migration 00149 not applied: %v", table, col, err)
				continue
			}
			if dt != spec.dt {
	...
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Errorf("replay of migration 00149 should be a no-op, got: %v", err)
	}
