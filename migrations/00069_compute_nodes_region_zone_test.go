...
// Migration-apply test for 00069 (multi-box placement scheduler:
...
//
//   1. The migration set applies cleanly through 00069.
...
func TestMigrations_00069_ComputeNodesRegionZone(t *testing.T) {
	...
	//     Region/zone are deliberately NOT NULL=YES so pre-00069
	...
	for _, col := range []string{"region", "zone"} {
		...
		if nullable != "YES" {
			t.Errorf("compute_nodes.%s is_nullable = %q, want YES (nullable so pre-00069 rows accept the schema)", col, nullable)
		}
	}
	...
	//     operator-added rows accept the 00069 schema without a
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00069-no-region-test', 'tcp://127.0.0.1:1', 1, 256, 1, 256, true)
	`); err != nil {
