...
-- Index for the list-obsolete query (app-scoped, status filter).
CREATE INDEX IF NOT EXISTS deployments_app_status_cancelled_idx
  ON deployments (app_id, status, cancelled_at DESC)
  WHERE cancelled_at IS NOT NULL OR deleted_at IS NOT NULL;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP INDEX IF EXISTS deployments_app_status_cancelled_idx;
ALTER TABLE deployments
  DROP CONSTRAINT IF EXISTS deployments_cancel_reason_check;
ALTER TABLE deployments
