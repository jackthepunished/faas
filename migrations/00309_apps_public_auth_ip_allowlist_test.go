...
// Migration-apply test for 00309 (ADR-118 — per-app ingress IP
// allowlist, extends apps.public_auth_mode with 'ip_allowlist').
// Pins the contract:
//
//   1. The migration set applies cleanly through 00309.
...
// TestMigrations_00309_AppPublicAuthIPAllowlist pins the
// ingress-IP-allowlist contract from ADR-118. Six named scenarios:
//
//   - ApplyThrough0309: the full migration set applies cleanly
//     through 00309 (regression: missing slot between 1 and 309
...
func TestMigrations_00309_AppPublicAuthIPAllowlist(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00309.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (regression: missing migration slot between 1 and 309)", err)
	}
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00000000-0000-0000-0000-000000000309',
