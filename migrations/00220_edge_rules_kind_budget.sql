-- filename: 00220_edge_rules_kind_budget.sql
-- +goose Up
...
--
-- Plan posture mirrors kind=validate / kind=limit: open to Free and
-- every other plan (no IsPaidOnly change). The default
-- budget when no rule matches is the per-plan budget on the plan
-- row (`plan.request_budget_ms`, default 0 → falls back to
-- api.RequestBudgetDefault = 3 s). Per-app overrides via
-- kind=budget take precedence over the plan default at request
-- time; the allow_override_header is the per-customer-tunable
-- knob (default `x-faas-budget-ms`).
--
...
--
-- DROP+ADD pair is canonical (migrations/00219_edge_rules_kind_limit.sql
...
-- "post-rewrite = pre-rewrite ∪ {new values}". PR-B / PR-D / PR-F
-- did this for validate, limit, etc.; this migration extends the
-- same convention to include 'budget'.
...
-- reverse and 00214's reverse: a downgrade should never silently
-- delete rule rows, but the CHECK drop-and-re-ADD will reject the
-- narrower re-add before any row is touched, so an operator sees
-- the problem at downgrade time, not silently later.
ALTER TABLE edge_rules DROP CONSTRAINT IF EXISTS edge_rules_kind_check;
ALTER TABLE edge_rules ADD CONSTRAINT edge_rules_kind_check
  CHECK (kind IN ('route', 'rewrite', 'redirect', 'headers',
                  'cors', 'jwt', 'ip', 'validate', 'limit'));
