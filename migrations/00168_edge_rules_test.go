...
// Migration-apply test for 00168_edge_rules.sql (ADR-089 / PR 1 of
...
//
// Seed UUIDs carry the slot number in the last group (`...000168`,
// `...000268`, `...000368`) so a reader scanning the test fixtures
...
func TestMigrations_00168_EdgeRules(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00168.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (regression: missing migration slot between 1 and 168)", err)
	}
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00000000-0000-0000-0000-000000000168',
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00000000-0000-0000-0000-000000000268',
	...
	if err := pool.QueryRow(ctx, `
		select match_path, match_methods, priority, enabled, kind
		  from edge_rules
		 where id = '00000000-0000-0000-0000-000000000468'
	...
	if err := pool.QueryRow(ctx, `
		select count(*) from edge_rules
		 where app_id = '00000000-0000-0000-0000-000000000268'
	...
	if _, err := pool.Exec(ctx, `
		delete from apps where id = '00000000-0000-0000-0000-000000000268'
	`); err != nil {
