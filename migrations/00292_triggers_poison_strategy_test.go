...
// Migration-apply test for 00292_triggers_poison_strategy.sql
// (per-trigger Kafka poison-record handling, audit #10 from PR #910).
//
// Pins:
//
//  1. Migration set applies cleanly through 00292 (no goose
...
func TestMigrations_00292_TriggersPoisonStrategy(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply cleanly.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (PR follow-up failure mode: missing migration slot between 00291 payload_max and 00292 poison_strategy)", err)
	}
