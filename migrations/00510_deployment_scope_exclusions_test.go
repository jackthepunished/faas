...
// Migration-apply test for 00510 (deployment_scope_exclusions
// table — ADR-124 follow-up #3 persistent --exclude history).
//
// Pins the load-bearing contracts:
//
//  1. The migration set applies cleanly through 00510; the new
...
//     00510_deployment_scope_exclusions.sql header).
...
// TestMigrations_00510_DeploymentScopeExclusions is the umbrella
// test for the new persistent --exclude history table. Each
// sub-test pins one of the contracts enumerated in the header.
func TestMigrations_00510_DeploymentScopeExclusions(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply the full migration set; 00510 is the new tail.
	// A regression that drops a slot between 1 and 510 surfaces
	// here before we get to the per-assertion pins.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (regression: missing migration slot between 1 and 510)", err)
	}
