...
// Migration-apply test for 00221_edge_rules_kind_geo.sql
...
//     migration 00221 rewrites this exact name; if a future
//     migration renames the inline CHECK in 00192, this pin +
//     the DROP+ADD in 00221 must update together (silent breakage
//     here means 00221 becomes a no-op).
//
...
func TestMigrations_00221_EdgeRulesKindGeo(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00221. The apply_walk_test.go pin auto-picks
	// up the new slot the next time `make embed-migrations` runs.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (regression: missing migration slot between 00192 and 00221)", err)
	}
	...
	// `<table>_<column>_check`). The 00221 migration rewrites
	// this exact name; if a future migration renames the inline
	// CHECK in 00192, this pin + the DROP+ADD in 00221 must update
	// together. Silent breakage here means 00221's DROP IF EXISTS
	...
	if constraintName != "edge_rules_kind_check" {
		t.Errorf("edge_rules kind CHECK named %q, want %q (regression: 00221's DROP+ADD targets this name; if 00192 renames the inline CHECK, both must update together)",
