...
--
-- 00111_reserve_slot.sql — slot reservation placeholder
...
--
-- Slot 111 is held by open PR #540 (test(state): raise pkg/state
-- coverage to 86% + fix latent PgStore bugs) with a real
-- migration `00111_webhook_deliveries.sql`. This reservation
-- bridges the gap on this branch so the embedded set stays
-- contiguous 1..114 while PR #540 is still open.
--
-- Whichever side lands first (PR-C with the 111 reservation +
-- 114 real migration, or PR #540 with its real 111 migration +
-- 112 reservation), the other drops its reservation on rebase.
