...
// Migration-apply test for 00166_edge_rules.sql (ADR-089 / PR 1 of
...
//
// Seed UUIDs carry the slot number in the last group (`...000166`,
// `...000266`, `...000366`) so a reader scanning the test fixtures
...
func TestMigrations_00166_EdgeRules(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Apply through 00166.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (regression: missing migration slot between 1 and 166)", err)
	}
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00000000-0000-0000-0000-000000000166',
	...
	if _, err := pool.Exec(ctx, `
		...
		values ('00000000-0000-0000-0000-000000000266',
	...
	if err := pool.QueryRow(ctx, `
		select match_path, match_methods, priority, enabled, kind
		  from edge_rules
		 where id = '00000000-0000-0000-0000-000000000466'
	...
	if err := pool.QueryRow(ctx, `
		select count(*) from edge_rules
		 where app_id = '00000000-0000-0000-0000-000000000266'
	...
	if _, err := pool.Exec(ctx, `
		delete from apps where id = '00000000-0000-0000-0000-000000000266'
	`); err != nil {
