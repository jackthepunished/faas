...
// Migration-apply test for 00228_apps_maintenance_mode.sql
// (ADR-091 amendment — D18 per-kind extension, PR-A fence + PR-B
// runtime + PR-C rollout-closer).
//
// Pins:
//
//  1. Migration set applies cleanly through 00228 (no goose
...
func TestMigrations_00228_AppsMaintenanceMode(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Run the full migration set. 00228 should land last.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (PR follow-up failure mode: missing migration slot between 00227 kind=maintenance and 00228 apps.maintenance_mode)", err)
	}
	...
	if err != nil {
		t.Fatalf("query apps.maintenance_mode column: %v (the 00228 ADD COLUMN IF NOT EXISTS must have landed)", err)
	}
	if dataType != "boolean" {
	...
	`).Scan(&indexDef)
	if err != nil {
		t.Fatalf("query apps_maintenance_mode_idx: %v (the 00228 CREATE INDEX IF NOT EXISTS must have landed)", err)
	}
	...
	if err != nil {
		t.Fatalf("query apps_maintenance_mode_notify trigger: %v (the 00228 CREATE TRIGGER must have landed)", err)
	}
	...
	accountID := "00000000-0000-0000-0000-000000002228"
	appID := "00000000-0000-0000-0000-000000012228"
