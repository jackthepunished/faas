// migrations/00488_deployments_canary_step_started_at_not_null_test.go —
// pins the SAFE-RELEASES code-review hardening of the canary
// wall-clock gate. Migration 00488 backfills NULL
...
//
// Asserts:
//   1. The migration set applies cleanly through 00488 (and lands
//      00488 last).
...
func TestMigrations_00488_CanaryStepStartedAtNotNull(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// (1) Run the full migration set. 00488 should land last.
	if err := db.MigrateUp(ctx, pool); err != nil {
		t.Fatalf("db.MigrateUp: %v (PR follow-up failure mode: missing migration slot before 00488)", err)
	}
	...
	if insertErr == nil {
		t.Errorf("insert with NULL canary_step_started_at must fail post-00488 (NOT NULL violation); got nil")
	}
