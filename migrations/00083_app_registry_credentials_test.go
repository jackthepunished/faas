...
// Migration-apply test for 00083 (per-app private-registry Basic Auth,
...
//
// Slot note: HEAD is at 00082 (apps scaling policy, PR-A from #462), so
// 00083 is the next free slot at PR creation time. The migration is
// slot-agnostic — only the filename and the test function name carry
// the literal slot. If a sibling PR grabs 00083 first, renumber per
...
func TestMigrations_00083_AppRegistryCredentials(t *testing.T) {
	ctx := context.Background()
	pool := pgtest.Open(t)

	// Seed UUIDs carry the slot number in the last group (`...000083`,
	// `...000183`, `...000283`, `...000383`) so a reader scanning the
	...
	if _, err := pool.Exec(ctx, `
		delete from accounts where id = '00000000-0000-0000-0000-000000000283'
	`); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		where account_id = '00000000-0000-0000-0000-000000000283'
