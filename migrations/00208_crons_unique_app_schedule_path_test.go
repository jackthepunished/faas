...
// Migration-apply test for 00208_crons_unique_app_schedule_path.sql
// (issue #791 PR-E / ADR-090 closure).
//
// Pins:
//
//  1. The crons_app_schedule_path_unique constraint exists after applying
//     00208 — the ALTER TABLE applied cleanly (no goose panic, no
...
func TestMigrations_00208_CronsUniqueAppSchedulePath(t *testing.T) {
	...
	_, err := pool.Exec(ctx, `
		...
		values ($1, '00208-crons-unique-test@example.com', now())
