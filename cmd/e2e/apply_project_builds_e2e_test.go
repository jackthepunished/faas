...
// close-the-loop).
//
// The 11 cases here are the load-bearing contract:
//
...
// once for the whole themed file (TestApplyProject_Builds).
// Each subtest reuses the harness and seeds an isolated account
// via the harness's label-based SeedAccount dedupe.
...
// TestApplyProject_Builds is the single top-level test for this
...
func TestApplyProject_Builds(t *testing.T) {
	pool := pgtest.Open(t)
	if pool == nil {
		return
	}
	...
	t.Run("OneBuildPerWorkload", func(t *testing.T) {
		// Pins the bare contract: every added app gets
		// exactly one (deployment, build) row pair. This is
		// the assertion that catches the gap PR-A closes —
		// pre-PR-A the count was zero.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-1perapp")
	...
	t.Run("KindTarball", func(t *testing.T) {
		// Pins that the build row's kind matches the
		// deployment row's kind and both are 'tarball'. The
		// MemStore/PgStore parity trap (plan §A4) would
		// surface here if anyone reintroduced an empty kind
		// default.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-kind")
	...
	t.Run("DeploymentStatusBuilding", func(t *testing.T) {
		// Pins that the helper flips the deployment status
		// to 'building' after the row lands (mirrors
		// cmd/apid/deploy_inputs.go:196). The dashboard keys
		// off this — a missing flip would surface as
		// 'pending' rows that builderd never claims (it
		// filters on status='queued' for the build row, not
		// the deployment status).
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-building")
	...
	t.Run("StagedTarballRootedAtRootDir", func(t *testing.T) {
		// Pins that the per-workload tarball is rooted at
		// RootDir, not the repo root. Without this, the
		// staging would produce identical tarballs for every
		// workload and the workload-isolation guarantee
		// would be lost.
		//
		// Strategy: build a tarball with a sentinel file at
		// RootDir that ONLY that workload's tarball should
		// contain, plus a shared file at the repo root.
		// After apply, find each workload's staged tarball
		// and check the sentinel is present.
		spoolRoot := t.TempDir()
		h2 := e2etest.StartWithEnv(t, pool, e2etest.APID, []string{
			"FAAS_SPOOL_ROOT=" + spoolRoot,
		})
		key := h2.SeedAccount(context.Background(), api.PlanPro, "builds-rooted")
		// Fixture: api's RootDir is "services/api" so the
		// per-app tarball should include only files under
		// that prefix. Sentinel markers per workload make it
		// possible to assert.
		entries := []struct{ name, body string }{
		...
		_ = gz.Close()
		tarBytes := buf.Bytes()
		ar := applyProjectMultipart(t, h2, key, "rooted", "", tarBytes)
		if len(ar.Apps) < 2 {
			t.Fatalf("need at least 2 apps, got %d", len(ar.Apps))
		}
		// Walk spoolRoot and find each app's tarball; verify
		// the matching sentinel file is inside and the
		// cross-sentinel is absent.
		var tarballs []string
		if err := filepath.Walk(spoolRoot, func(path string, info os.FileInfo, err error) error {
		...
		// Each tarball should contain exactly one of the
		...
		for _, path := range tarballs {
			...
			// Exactly one of the two sentinels should be
			// present per workload's tarball. None of the
			// tarballs should contain BOTH sentinels (that
			// would mean RootDir wasn't honoured).
			if hasAPI && hasWorker {
	...
	t.Run("BuildQueuedNotifyFires", func(t *testing.T) {
		// Pins that the helper's Notify(BuildQueued)
		// actually fires. We do not subscribe to the channel
		// here (that'd require a real LISTEN connection);
		// we inspect the audit log row the apply path emits
		// on each successful enqueue. (The audit row is the
		// durable "the notify was sent" marker; a missing
		// row catches both a failed notify AND a missing
		// audit row.)
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-notify")
		ar := applyProjectMultipart(t, h, key, "notify", "", buildProjectFixture(t))
		// Audit row emitted by the helper on every enqueue.
		// The audit taxonomy is project.build.enqueued (see
		// plan §B).
		var auditCount int
		err := pool.QueryRow(context.Background(),
	...
	t.Run("ApplyResponseBuildsSlice", func(t *testing.T) {
		// Pins the wire shape of ApplyResponse.Builds:
		// every workload has a (deployment_id, build_id)
		// pair, no errors.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-wireshape")
	...
	t.Run("PartialFailureLeavesOthersIntact", func(t *testing.T) {
		// Pins the partial-success design (plan §A5). To
		// force one workload to fail staging we delete its
		// source files from the extracted tree before apply.
		// That's an invasive setup, so we use a more
		// surgical approach: stuff a workload with a
		// RootDir that doesn't exist in the tree.
		// RepackageRootTree will fail; the apply loop must
		// continue and enqueue the other workloads.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-partial")
		// Fixture with one valid workload (api) and a
		// 'ghost' reference in compose that won't resolve.
		// The reposcan layer just reads the compose file,
		// so this passes scanning; the apply-time staging
		// walk is where it breaks.
		entries := []struct{ name, body string }{
		...
		// At minimum: the api workload has no error; the
		// ghost workload (if scanned as a workload) has an
		// error.
	...
	t.Run("UnchangedWorkloadsGetNoBuild", func(t *testing.T) {
		// Pins that a 2nd apply against an unchanged repo
		// does NOT create new build rows for the unchanged
		// workloads. (The githubd dispatcher's
		// path-filtered fan-out model only enqueues builds
		// for apps whose RootDir or WorkloadName drifted
		// across applies.)
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-unchanged")
		ar := applyProjectMultipart(t, h, key, "unchanged", "", buildProjectFixture(t))
		// Re-apply the SAME tarball. The apps already
		// exist; this is the "no drift" case. A correct
		// implementation creates zero new build rows (only
		// changed workloads re-build).
	...
	t.Run("BuildIDIsUUIDv7", func(t *testing.T) {
		// Pins the build id shape: builderd's poll loop
		// (state.Store.ClaimNextQueuedBuild) keys off the
		// UUID format and dashboards filter on it. The
		// apply path's build IDs come from state.Build.ID —
		// pin the shape so a future refactor doesn't break
		// the wire contract.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-uuid")
	...
	t.Run("DeploymentIDStableAcrossReapply", func(t *testing.T) {
		// Pins that the second apply's deployment ID is
		// different from the first (every apply produces a
		// fresh deployment row). Catches a bug where the
		// helper accidentally returns a cached ID.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-stable")
	...
	t.Run("NoDoubleNotifyOnUnchangedApply", func(t *testing.T) {
		// Pins that a 2nd apply with the same source
		// doesn't fire a duplicate build_queued notify for
		// every workload. The notify is best-effort and the
		// durable recovery
		// (state.Store.ClaimNextQueuedBuild) would dedup,
		// but the audit row count is the canonical pin.
		key := h.SeedAccount(context.Background(), api.PlanPro, "builds-nonotify")
		...
		// Single project, single apply of N apps → N
		// deployments. If the 2nd apply incorrectly
		// re-enqueued unchanged workloads, deps would be 2N.
		if deps == 0 {
			t.Fatalf("no deployments (apply didn't enqueue anything)")
		}
		// The apply path's added+changed diff should produce
		// 0 new builds on the 2nd identical apply. The
		// fixture doesn't drift, so Result.Added +
		// Result.Changed on the 2nd apply is empty → no new
		// enqueues. We pin the lower bound: at least N
		// deployments (1 per app) and at most N (no
		// double-build on unchanged apply).
		var appCount int
		if err := pool.QueryRow(context.Background(),
			`select count(distinct app_id) from deployments d join apps a on a.id = d.app_id where a.project_id = $1`,
			projID).Scan(&appCount); err != nil {
			t.Fatalf("count distinct apps: %v", err)
		}
		depCount := deps
		// One deployment per app — the canonical 1:1
		// invariant.
