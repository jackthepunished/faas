...
func TestCheckSecrets_AllPresentNoDrift(t *testing.T) {
	...
	writeSecretFile(t, filepath.Join(storageDir, "box-age-key"), []byte("placeholder box-age-key"), "0400")
...
func TestCheckSecrets_MissingHostAge(t *testing.T) {
	...
	storageDir := filepath.Join(dir, "storage-box")
	writeSecretFile(t, "/etc/faas/secrets/session.key", []byte(strings.Repeat("a", 64)), "0400")
	writeSecretFile(t, filepath.Join(storageDir, "box-age-key"), []byte("placeholder"), "0400")
	writeSecretFile(t, filepath.Join(storageDir, "rclone.conf"), []byte(`{}`), "0400")
