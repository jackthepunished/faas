...
//
// P2a (force-park) + P2b (force-cold-boot) wired to the schedd
// gRPC socket. Both subcommands dial
...
//
// The two subcommands are intentionally thin wrappers — they
// call the same gRPC RPCs that pkg/meterd already calls
// (`scheddgrpc.Client.ParkInstance`,
// `scheddgrpc.Client.ForceColdBootNextWake`) and print a
...
// (commands_compute_nodes.go:245-280): both subcommands require
// --yes as a tripwire against operator fat-fingering.
package main
...
// Same (args []string) int signature every other dispatch* arm
// uses (see commands_release.go:cmdReleaseDispatch).
func cmdInstancesDispatch(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "gregalectl instances: missing subcommand; want force-park|force-cold-boot")
		return 2
	}
	switch args[0] {
	case "force-park":
