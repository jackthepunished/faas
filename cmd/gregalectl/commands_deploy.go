...
// no-op stub via setGitRunner.
var gitRunner = defaultGitRunner

func setGitRunner(fn func(repoRoot string, args ...string) error) {
	gitRunner = fn
}

// defaultGitRunner runs `git -C <repoRoot> <args...>` in the
// caller's repo.
func defaultGitRunner(repoRoot string, args ...string) error {
...
func addNodeExecute(p addNodeParams) (addNodeReport, int) {
	...
	if err := gitRunner(p.RepoRoot, "add", "deploy/ansible/host_vars/"+p.FQDN+".yml", "deploy/ansible/inventory/hosts.ini"); err != nil {
	...
	if err := gitRunner(p.RepoRoot, "commit", "-m", commitMsg); err != nil {
	...
	bootstrapCmd := []string{"sudo", "make", bootstrapTarget(p.Role)}
	...
	// the CLI directly.
	if code := cmdComputeNodesAdd([]string{"--from-file=" + scratch.Name(), "--json"}); code != 0 {
...
// gitHeadSHA returns the HEAD commit's SHA. Used by the report.
