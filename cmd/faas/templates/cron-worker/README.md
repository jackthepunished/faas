...
the customer can `faas logs <slug>` and see structured progress across
cold boots.
...
```sh
faas secrets set --app <slug> QSTASH_TOKEN=<qstash-signing-key> \
...
first invocation with the exact `faas secrets set` command — the
runtime surfaces it as a 500 to QStash (which logs it) and the
customer sees the actionable hint in `faas logs <slug>`.
