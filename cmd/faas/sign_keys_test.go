...
// (`faas sign-keys init|rotate|status`). The dispatcher lives in
...
func TestCmdSignKeys_NoArgs_PrintsUsage(t *testing.T) {
	...
	got := readStderr()
	if !strings.Contains(got, "usage: faas sign-keys") {
...
func TestCmdSignKeysInit_WritesBothFilesWithCanonicalModes(t *testing.T) {
	...
	// "operator can immediately restart faas-imaged without any
	// post-fixup" guarantee — the bootstrap.sh chown root:faas step
...
func TestCmdSignKeysInit_ExtraArgsRejected(t *testing.T) {
	...
	if !strings.Contains(readStderr(), "usage: faas sign-keys init") {
