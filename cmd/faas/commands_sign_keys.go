...
//
// The namespace `faas keys` is already taken by the customer-facing
...
// command `faas sign-keys` with three leaves:
...
func cmdSignKeys(args []string) int {
	if len(args) == 0 {
		PrintUsage(os.Stderr, "usage: faas sign-keys <init|rotate|status> [flags]", "sign-keys")
		return 1
	}
	switch args[0] {
	case subInit:
	...
	default:
		fmt.Fprintf(os.Stderr, "faas sign-keys: unknown subcommand %q (known: init, rotate, status)\n", args[0])
		return 1
	}
}
...
// `faas sign-keys rotate` does NOT silently overwrite (the operator
...
func newSignKeyFlags(name string, defaultForce bool) (*flag.FlagSet, *signKeyFlags) {
	...
	fs.StringVar(&f.signKey, "sign-key", cosign.DefaultSignKeyPath,
		"path to the private key (mode 0440 root:faas on the canonical install)")
...
// root:faas succeeds). `faas sign-keys init --force` is allowed
// for emergency re-init but the operator should normally use
// `faas sign-keys rotate` for that flow — `init --force` skips
...
func cmdSignKeysRotate(args []string) int {
	...
	if fs.NArg() != 0 {
		PrintUsage(os.Stderr, "usage: faas sign-keys rotate [flags]", "sign-keys")
		return 1
	}
	if err := writeKeyPair(f.force, f.signKey, f.verify); err != nil {
		return printErr("rotate failed", err)
	}
	PrintOK(osStdout, "Rotated %s and %s (force=%t)\n  Restart: systemctl restart faas-imaged faas-schedd",
...
func cmdSignKeysStatus(args []string) int {
	...
	if fs.NArg() != 0 {
		PrintUsage(os.Stderr, "usage: faas sign-keys status [flags]", "sign-keys")
		return 1
	}
