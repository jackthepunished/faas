...
//
// Mirrors the `faas sign-keys` test style (sign_keys_test.go): the
...
//     file set and a README that mentions the right `faas secrets
...
func TestCmdInit_AllTemplatesMaterialize(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if !strings.Contains(stdout.String(), "https://docs.DOMAIN/storage") {
...
// created via MkdirAll. This is the happy path for `faas init --path=./uploader`
// on a fresh checkout.
...
// TestCmdInit_NextStepsFor: pins the per-template `faas secrets set`
...
func TestCmdInit_NextStepsFor(t *testing.T) {
	cases := []struct {
		...
		{"s3-uploader", []string{"S3_BUCKET", "faas secrets set", "cd <dest>"}},
...
func TestCmdInit_PreservesStdoutBytes(t *testing.T) {
	...
	if !strings.Contains(captured.String(), "https://docs.DOMAIN/storage") {
