// Tests for the `faas usage summary [--month YYYY-MM]` subcommand.
...
// bare `faas usage` must continue to call Client.GetUsage (per-app
...
func TestCmdUsage_NoPositional_DispatchesToList(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/usage/summary":
			t.Errorf("dispatcher reached /v1/usage/summary for bare `faas usage`; want /v1/usage")
		default:
			http.Error(w, "no", 404)
		}
	}))
...
func TestCmdUsage_SummarySubcommand_DispatchesToSummary(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/usage":
			t.Errorf("dispatcher reached /v1/usage for `faas usage summary`; want /v1/usage/summary")
		default:
			http.Error(w, "no", 404)
		}
	}))
