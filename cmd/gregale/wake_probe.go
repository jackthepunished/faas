...
// reports whether the server tagged the response `x-gregale-wake: cold`.
...
// `x-gregale-wake: cold` header. Errors are reported separately so the
...
func probeWakeState(url string, timeout time.Duration) (cold bool, err error) {
	...
	return resp.Header.Get("x-gregale-wake") == "cold", nil
}
