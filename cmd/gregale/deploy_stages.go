...
// the SCHEMA CHECK in migrations/00300. A divergence here is a
...
func renderStageTicker(w io.Writer) *stageTicker {
	if len(stageOrder) != stageOrderClosedSet {
		panic(fmt.Sprintf("renderStageTicker: stageOrder has %d entries, want %d (closed set in pkg/state.AllStageNames + migrations/00300)", len(stageOrder), stageOrderClosedSet))
	}
