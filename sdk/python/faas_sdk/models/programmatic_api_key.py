from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

T = TypeVar("T", bound="ProgrammaticAPIKey")


@_attrs_define
class ProgrammaticAPIKey:
    """Freshly minted API key returned on the first
    request. The `plaintext` field is the only copy the
    caller will ever see — store it in `~/.config/faas/auth.json`
    immediately. The `id` is the row's UUID for later
    list/delete via `/v1/keys`.

    """

    plaintext: str
    """`fp_live_<48-hex>`. Returned ONCE."""
    prefix: str
    """`fp_live_<8-hex>` — the prefix used in the auth
    header for greppable identification.
    """
    id: str
    """API key row UUID."""
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        plaintext = self.plaintext

        prefix = self.prefix

        id = self.id

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "plaintext": plaintext,
                "prefix": prefix,
                "id": id,
            }
        )

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        plaintext = d.pop("plaintext")

        prefix = d.pop("prefix")

        id = d.pop("id")

        programmatic_api_key = cls(
            plaintext=plaintext,
            prefix=prefix,
            id=id,
        )

        programmatic_api_key.additional_properties = d
        return programmatic_api_key

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
