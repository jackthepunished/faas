...
the generator's `Client._client` (an `httpx.Client`) for a new
one whose `transport=` is the chain. The generated service
functions call `client.get_httpx_client().send(request)`, so the
chain sits on the data path for every public route.
"""
...
def install_chain(
    ...
    *,
    ...
    """Swap the generator's `Client._client` for one whose
    `transport=` is the BaseTransport chain.
    """
