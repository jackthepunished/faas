...
| A.5.12 | Classification of information | Implemented | Customer secrets classified as **restricted** (ADR-020, sealed at rest); audit log classified as **internal** (90-day retention per §5.1); app code classified as **customer-owned** (Controller's asset, not Gregale's). | |
...
| A.5.14 | Information transfer | Implemented | TLS 1.3 in transit (ADR-024, certmagic); HSTS + CSP with per-request nonce; `kernel.unprivileged_userns_clone=0`. | C-6.7. |
...
| A.5.21 | Managing information security in the ICT supply chain | Implemented | OCI image digest pinning (`pull_digest`); cosign signature enforcement (ADR-058); grype scan per deploy (PR-10, ADR-075). | |
...
| A.5.24 | Information security incident management planning | Implemented | `docs/runbooks/` per-family (FaasWakeLatencyHigh, FaasDaemonDown, FaasFailedLoginSpike, …); alertmanager routing (ADR-039 / §12.3). | C-4.2, C-7.4. |
| A.5.25 | Assessment & decision on information security events | Implemented | Per-instance liveness probe (ADR-079) — 3 consecutive non-2xx on vsock 1028 → destroy + cold-boot; 3 destroys in 300 s → `apps.status='evicted_cold'`. | C-4.1. |
| A.5.26 | Response to information security incidents | Implemented | Liveness exhaust path; webhook replay → 200 idempotent (ADR-042); session replay → 401 (ADR-039). DPA §10 breach notification (72 h). | C-7.4. |
...
| A.5.32 | Intellectual property rights | Implemented | Customer owns the deployed code (Controller asset, not Gregale's); Builder MicroVM boundary isolates build → deploy (ADR-003). | |
| A.5.33 | Protection of records | Implemented | `events` table retention 90 days (spec §5.1); `audit_log` retention forever (PR-6); PG `pgbackrest` encrypted backups retained 30 days. | |
...
| A.6.7 | Remote working | Implemented | All control-plane access over Tailscale / Wireguard overlay (spec §11, ADR-052 control-plane mTLS); no SSH keys handed out to customers. | |
...
| A.8.3 | Information access restriction | Implemented | `pkg/auth/middleware.RequireSession` is the single authentication seam; per-app egress allowlist (ADR-031, ADR-033); `faas-tenant.slice` cgroup scope. | C-6.6. |
...
| A.8.5 | Secure authentication | Implemented | `pkg/auth` (cookie + bearer + TOTP MFA per ADR-077); `keys.revoked_at` lineage (ADR-039 family). | C-6.1. |
...
| A.8.8 | Management of technical vulnerabilities | Implemented | Branch protection + Dependabot + grype per deploy (ADR-075, PR-10). | C-7.1. |
...
| A.8.11 | Data masking | Implemented | Customer secrets sealed at rest with host X25519 (ADR-020); sealed-secret envelope includes an integrity tag; no plaintext VALUES in Postgres. | C-1.1. |
| A.8.12 | Data leakage prevention | Implemented | Per-instance netns + egress deny list (SMTP 25/465/587, RFC1918, link-local, metadata); per-app egress allowlist (ADR-031/ADR-033). | C-6.6. |
| A.8.13 | Information backup | Implemented | Postgres `pgbackrest` daily to Hetzner Storage Box (encrypted with host X25519 age recipient, ADR-020); 30-day retention. | C-A1.2. |
| A.8.14 | Redundancy of information processing facilities | Planned | Multi-node HA (ADR-062 + ADR-066, M9 acceptance gate — **Type 2 only**); cross-node live migration in M9. | C-A1.2. |
| A.8.15 | Logging | Implemented | `events` table + `apid_audit_write_failures_total` + `schedd_audit_write_failures_total` (§5.1 + ADR-035); Prometheus metrics per spec §12 (ADR-036, ADR-041); Loki for log aggregation. | C-2.1, C-7.2, C-7.3. |
...
| A.8.20 | Networks security | Implemented | nftables default-drop inbound (spec §11); per-instance netns; per-app egress allowlist (ADR-031/033). | C-6.6. |
| A.8.21 | Security of network services | Implemented | TLS 1.3 at edge (ADR-024, certmagic); mTLS for control-plane gRPC (ADR-052). | C-6.7. |
...
| A.8.24 | Use of cryptography | Implemented | TLS 1.3 at edge; X25519 host age key for sealed secrets (ADR-020); AES-GCM session envelope (ADR-039); ChaCha20-Poly1305 sealed-secret envelope (ADR-020); SHA-256 for API-key hashing + HMAC verification; constant-time compare for HMAC. | C-6.7, C-1.1. |
...
| A.8.26 | Application security requirements | Implemented | Spec §11 is the application-security baseline; per-app `require_signed` flag (ADR-058); per-deployment `require_authn` flag (issue #560); per-app egress allowlist (ADR-031/033). | |
| A.8.27 | Secure system architecture & engineering principles | Implemented | `docs/faas_implementation_spec.md` §1–§6; `docs/scale_out_and_workload_classes.md`; ADR-009 (identical inner network, load-bearing). | |
