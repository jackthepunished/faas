...
| `webhook.replay_rejected` | gatewayd `audit.go`, apid `audit.go` | `{provider, delivery_id}` — emitted on the replay path (`delivery_id` re-appears within the 5-minute TTL window). Subject: customer account id from the apid Stripe/Paddle paths; `nil` from the gatewayd GitHub path (no account id at the edge). ADR-041.
