...
- Auth: API keys (`fp_live_…`, SHA-256 stored), per-user; sessions for the dashboard later. Every key scoped to an account. **Tenancy (ADR-061, issue #190 / IAM-6):** API keys are additionally bound to one organization in addition to their existing scopes (PR 6); the customer's tenant is the `orgs` row, not the `accounts` row. Effective permission is the intersection of key scopes and the creator's current membership role. Legacy keys without an org binding are surfaced via `CodeOrgAPIKeyRequiresOrg` (PR 6) and re-minted under `/v1/orgs/{slug}/keys`. Cookie sessions remain account-bound; org selection is per-request via URL slug. The cross-tenant rule is the existing IDOR-safe `LoadApp` pattern — cross-org access returns 404, never 403.
...
| G14 | **Account is the only tenant — no org / team abstraction** — `accounts.id` owns resources, carries the plan and provider identifiers, scopes gateway limits, and drives scheduler / metering admission. A 5-engineer team has no way to share a single billing identity, role-gate access, or rotate a colleague's key without nuking the host session key. Sales stalls at the first team-sized customer | **RESOLVED (ADR-061, issue #190 / IAM-6, 9-PR staged rollout):** introduce an `orgs` table as the tenant for ownership, plan, quota, billing, and audit. Keep `accounts` as a human authentication identity that belongs to one or more orgs. Path-scoped APIs under `/v1/orgs/{org_slug}/...`, automatic personal-org compatibility for every account, and one existing plan/quota pool per org. Paid per-seat charging and SSO are deferred to follow-up issues. Each `Limits` row gains `OrgMembersMax` and `OrgPendingInvitationsMax` (PR 1 ships 0/0 fail-closed; PR 2 populates from the financial model). Twelve new RFC 7807 codes (`org_not_found`, `org_slug_invalid`, `org_slug_taken`, `org_member_cap_exceeded`, `org_invitation_cap_exceeded`, `org_role_forbidden`, `org_already_member`, `org_invitation_invalid`, `org_invitation_expired`, `org_last_owner`, `org_personal_immutable`, `org_api_key_requires_org`) added to `pkg/api/errors.go` with full `StatusForCode` mapping. Roles: `owner`, `admin`, `developer`, `viewer`, `billing` — exactly one owner per non-personal org; ownership transfer is the only way to vacate the role. Personal orgs are immutable. Cookie sessions remain account-bound; org selection is per-request via URL. Detailed ownership inventory checked in at `docs/iam-6-ownership-inventory.md` | M8 (IAM-6 PR 1–9) |
