...
- **Streaming posture.** The rule carries an optional `max_body_bytes_streaming ≤ 100 MiB` (matching `pkg/api.RawStreamMaxRequestBytes` for ADR-080 raw-bridge parity). The DTO + apid-Validate + cmd-side compile clamps are all in place, but **runtime enforcement of the streaming field is deferred to a follow-up PR** — the load-bearing reason: detecting the streaming opt-in path at the §4.1.2.13 slot requires `isAcceptJSON(r.Header.Get("Accept"))` + `h.streamingEnabled` + `app.StreamingEnabled`, all of which live further down the chain at handler.go:3019. Pulling that detection above the §4.1.2.13 slot would tangle the rule-application order with the proxy configuration. Until that ships, `max_body_bytes_streaming` is declared and persisted, but the applier falls back to `max_body_bytes` for both paths. State explicitly in ADR-091 D24 §6 so a future operator doesn't confuse "the field exists in the API" with "the field is enforced at request time".
