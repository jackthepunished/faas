...
- Plan gate: `pkg/api/limits.go` gains `Plan.RouteMetricsEnabled()`
  (create-time default) and `Plan.RouteMetricsResponseAllowed()`
  (PATCH-time gate) mirroring `Plan.WebSocketEnabled()` at the same
  site. Limits table rows: Free = false, Hobby = true, Pro = true,
  Scale = true. Free-tier attempts return
  `plan_route_metrics_not_allowed` (403).
...
|---|---|---|---|
| `gateway_requests_by_route_total` | `{app, plan, route, code}` | Counter | 50 × 60 × ~5 = ~15,000 (unchanged from the existing 4-class rollup × N apps, distributed over 60 codes) |
| `gateway_request_duration_by_route_seconds` | `{app, route, class}` | Histogram (11 buckets + `_sum` + `_count`) | 50 × 4 × 14 = **2,800** per opt-in app |
| `gateway_request_failures_by_route_total` | `{app, plan, route, code}` | Counter | subset of `gateway_requests_by_route_total` (status ≥ 400) |
...
- alert: GatewayWildcardRoute
  expr: |
    sum by (app) (rate(gateway_requests_by_route_total{route="__route_other__"}[10m])) > 1
  for: 10m
...
| `pkg/api/limits.go` | Constant `route_metrics_per_app_cap = 50`. `Plan.RouteMetricsEnabled()` (create-time default) + `Plan.RouteMetricsResponseAllowed()` (PATCH-time gate) mirroring `Plan.WebSocketEnabled()`. Limits rows: Free=false, Hobby=true, Pro=true, Scale=true. |
