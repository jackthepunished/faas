...
| `POST /dashboard/account/set-password` | `dashboardChain → sessionAuth → postSetPassword` | `dashboardChain → sessionAuth → requireStepUpHandler(5m) → postSetPassword` — **superseded by ADR-140**: the gate is back off the mount and the handler picks the proof (fresh step-up, `current_password`, or 403 for MFA-enrolled accounts) |
