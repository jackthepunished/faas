...
3. **`(app_id, schedule, path)` UNIQUE constraint** — `migrations/00209_crons_unique_app_schedule_path.sql` (renumbered twice at push time: 00207 collided with PR #826 + PR #836; 00208 collided with PR #836's deployments_scope) enforces the tuple the manifest dedupe primitive has been asserting all along. A customer-managed `triggers:` manifest with two entries that collide on this tuple now fails fast at INSERT instead of producing a duplicate crons row.
