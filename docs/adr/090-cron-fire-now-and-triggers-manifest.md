...
3. **`(app_id, schedule, path)` UNIQUE constraint** — `migrations/00208_crons_unique_app_schedule_path.sql` (renumbered from 00207 at push time because PR #826 and PR #836 also claim 00207) enforces the tuple the manifest dedupe primitive has been asserting all along. A customer-managed `triggers:` manifest with two entries that collide on this tuple now fails fast at INSERT instead of producing a duplicate crons row.
