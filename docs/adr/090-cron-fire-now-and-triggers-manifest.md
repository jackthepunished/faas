...
3. **`(app_id, schedule, path)` UNIQUE constraint** — `migrations/00207_crons_unique_app_schedule_path.sql` enforces the tuple the manifest dedupe primitive has been asserting all along. A customer-managed `triggers:` manifest with two entries that collide on this tuple now fails fast at INSERT instead of producing a duplicate crons row.
