...
migration at slot 00358 documents the cap; schema unchanged.
...
- `worktree-feat-deploys-stages-prod-ready` (single PR,
  ...
  - Commit 2: C1 (retention). Doc-only migration at slot
    00358; trim happens Go-side at the existing read-modify-
  ...
  - Commit 3: C2 (retry endpoint + CLI). One new apid route +
    ...
    reservation; main has likely moved past 00358 per recent
    slot-dance chains — slot renumbered six times
    (00330 → 00346 → 00348 → 00352 → 00353 → 00356 → 00358)
    during rebases as adjacent PRs landed real migrations at
    00346 (issue #977 annotation, PR #984), 00352-00356 (PR #1017
    alert presets catalog — 5 real migrations), 00357 (PR #990
    ADR-117 PR-C app_secret_value_hash), and 00358 is the next
...
touched files). 1 new migration (slot 00358, doc-only).
