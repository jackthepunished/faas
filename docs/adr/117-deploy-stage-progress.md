...
migration at slot 00356 documents the cap; schema unchanged.
...
- `worktree-feat-deploys-stages-prod-ready` (single PR,
  ...
  - Commit 2: C1 (retention). Doc-only migration at slot
    00356; trim happens Go-side at the existing read-modify-
  ...
  - Commit 3: C2 (retry endpoint + CLI). One new apid route +
    ...
    reservation; main has likely moved past 00356 per recent
    slot-dance chains — slot renumbered five times
    (00330 → 00346 → 00348 → 00352 → 00353 → 00356) during
    rebases as adjacent PRs landed real migrations at 00346
    (issue #977 annotation, PR #984), 00347-00351 (PR #1017
    alert presets catalog — 5 real migrations), 00352 (PR #990
    ADR-117 PR-C app_secret_value_hash), 00353 (PR #1005
    api-contract-diff deployment_openapi_snapshots), 00354-00355
    (PR #1006 SAFE-RELEASES deployment_audit + backfill), and
    00356 is the next free slot above all open PRs.
...
touched files). 1 new migration (slot 00356, doc-only).
