...
| 093 | schedd wake-phase telemetry — `schedd_wake_phase_duration_seconds{app, phase}` HistogramVec on schedd, closed-set `phase ∈ {admit_to_rpc, rpc_call, rpc_to_running}`; spec §6.3 buckets + low-end `0.01` for `admit_to_rpc`; `wake_id` attached as `prometheus.Exemplar` (no label cardinality cost) so operators join to the `events` table and to `gateway_wake_latency_seconds` on the gateway side; new instrumentation at engine.go:1570 / :1744 / :1759 / :1892; three new observations per wake on the success path (error paths use `events.BootFailed - BootStarted`); spec §6.3 paragraph + §12.1 row updated; mirrors the existing `schedd_guest_init_duration_seconds{app, runner}` pattern at metrics.go:1172-1177 | accepted | P1B (P1 autoscaling PR-cluster: P1C + P1A + P1B; ADR-093 ships as commit 1 of the cluster) |
