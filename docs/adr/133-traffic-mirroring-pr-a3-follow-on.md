...
4. **`feat(state): mirror_invocation_summary rollup + retention sweep`** — `migrations/00510_mirror_invocation_summary.sql` (PRIMARY KEY `(rule_id, hour_bucket)`, additive-merge UPSERT), `pkg/mirror/rollup.go` (`RollupOnce` / `SweepOldLedgerRows` / `RollupLoop`), wired into `cmd/schedd/main.go` next to the scale-up triggers.
