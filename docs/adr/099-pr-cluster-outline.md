...
| **PR-A schema** | `migrations/00245_jobs.sql` (3 tables), `migrations/00246_instances_kind_job.sql` (instances widening), `migrations/00247_usage_minutes_app_nullable.sql` (metering widening), `migrations/00248-00253_reserve_slot.sql` (6 fences), `docs/adr/099-pr-cluster-outline.md` (this file) | None. Pure DDL; no code path reads the new tables yet. Default backfill (`kind='wake'`, `meter_kind='app'`) preserves the existing wake/build paths. | ~10 min |
...
4. No direct cross-component call. apid writes PG → meterd reads →
   ...
   introduced in 00245). No apid→schedd gRPC.
...
8. Slot fence discipline (`cross-pr-slot-gate-reservation-fence-pattern`,
   `pr-849-adr-092-pr-a-slot-chase-cluster`). 00248-00253 carry the
   PR-B/C/D/E/F coordination headroom.
...
`git ls-tree origin/main migrations/` at PR-A push time had 00237 as
the highest merged slot. Open PRs #864 (reqbudget) and #887 (throttle)
both claim 00244 (unresolved collision). Issue #879 PR-cluster
(merged as #884) fenced 00238-00243 for its own PR-A target
(`00243_tenant_surfaces.sql`).

PR-A renumbered from the plan's "00231-00240 reserved" assumption to
**00245-00253** (4 migrations + 6 fences). Skipping 00244 keeps the
reqbudget + throttle siblings undisturbed; 00245 is the next free
slot.

| Slot | File | PR owner |
|---|---|---|
| 00245 | `00245_jobs.sql` | PR-A (this PR) |
| 00246 | `00246_instances_kind_job.sql` | PR-A |
| 00247 | `00247_usage_minutes_app_nullable.sql` | PR-A |
| 00248 | `00248_reserve_slot.sql` | PR-B (sqlc regen, no real migration; fence still useful for cross-PR coordination) |
| 00249 | `00249_reserve_slot.sql` | PR-C (1-2 migrations: dispatch tick + Watchdog exit-code DGRAM port) |
| 00250 | `00250_reserve_slot.sql` | PR-D (no migration; OpenAPI regen only) |
| 00251 | `00251_reserve_slot.sql` | PR-E (no migration; CLI + e2e + dashboard) |
| 00252 | `00252_reserve_slot.sql` | PR-F (no migration; docs only) |
| 00253 | `00253_reserve_slot.sql` | spare buffer for hot-fix or future amendment |
