...
**Operator staging:** same recipe as the bookworm base — `docker build`
+ `mkfs.ext4 -O '^has_journal' -d <staging>` — copied to
`/srv/fc/base/runner-go124-alpine.ext4` on the EX44. imaged does not
auto-stage runtime bases (only the builder base is auto-staged).
...
<!-- CI status: migration 00037_apm_runtime_go124.sql verified on PR #201; 00043_app_runtime_go124_alpine.sql added in Tier 2 PR. -->
