...
and `TestManifestFromImageConfig_IgnoresOCIEntrypointWithoutCmd`
(negative: an image without `Cmd` produces an empty entrypoint that
fails `manifest.Validate()` with `"empty entrypoint"`). Both tests
live in `pkg/imaged/handler_test.go`.
