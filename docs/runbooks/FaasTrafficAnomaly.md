...
`apid_request_failures_total{account_id, route}` for the per-account
error-rate view, issue #278).

Severity: page (`mode=fleet`), warn (`mode=account`). The `family` label
is `traffic_anomaly`, so the existing `family`-based inhibition /
silencing rules compose with this alert.

## Symptom

The page fires when one of four alert conditions holds for the
...
in the JSON) that read from the recording rules. For ad-hoc
