...
func RenderText(w io.Writer, d Diff) {
	if d.Slug != "" {
		fmt.Fprintf(w, "Deployment diff for %q\n\n", d.Slug)
	}
	...
	if len(erRows) > 0 {
		fmt.Fprintln(w, "Routes / edge rules:")
	...
	if len(errors) > 0 {
		fmt.Fprintln(w, "Plan-quota breaks (gate fires):")
	...
	if d.HasBlockingBreaks() {
		fmt.Fprintln(w, "Fix before deploy.")
	} else if len(d.Changes) > 0 {
		fmt.Fprintln(w, "Ready to deploy.")
	} else {
