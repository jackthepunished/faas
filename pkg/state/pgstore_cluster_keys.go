...
// public.cluster_signing_keys (resolved via search_path). KeyID is the kid header schedd
	...
	_, err := s.pool.Exec(ctx, `
		...
		ON CONFLICT (id) DO UPDATE
		    SET key_id         = EXCLUDED.key_id,
		        ...
		        rotated_at     = COALESCE(cluster_signing_keys.rotated_at, now()),
		        retired_at     = CASE
...
func (s *PgStore) DeleteClusterSigningKey(ctx context.Context) error {
	tag, err := s.pool.Exec(ctx, `DELETE FROM cluster_signing_keys WHERE id = 1`)
	if err != nil {
