...
// land across migrations 00255, 00256, 00257, 00562, 00563, 00565, 00566,
// 00567, 00568, 00569 — this file defines the Go-side domain types + the
...
//     00562, 00563, 00565, 00566, 00567, 00568, 00569) --------------

// Job is one row of public.jobs (migrations/00255 + 00563 for command).
...
// by 00563 (text[], capped at 64 entries by jobs_command_min_chk).
//
...
type Job struct {
	...
	CreatedAt      time.Time
	UpdatedAt      time.Time
	Command        []string // migrations/00563
}

// JobRun is one row of public.job_runs (migrations/00255 + 00565 for
...
type JobRun struct {
	...
	TasksCancelled  int
	TasksRunning    int
	DeadLetterCount int // migrations/00565
	StartedAt       *time.Time
	FinishedAt      *time.Time
	CreatedAt       time.Time
}

// JobTask is one row of public.job_tasks (migrations/00255 + 00562 for
// exit_code + next_attempt_at + relaxed CHECK constraints; 00565 for
...
// claimed ⇒ NOT NULL; terminal ⇒ either, see migrations/00562).
type JobTask struct {
	RunID          string
	TaskIndex      int
	...
	FinishedAt     *time.Time
	CreatedAt      time.Time
	NextAttemptAt  *time.Time // migrations/00562 — retry backoff gate
	LeaseToken     *string    // migrations/00565
	LeaseExpiresAt *time.Time // migrations/00565
	LastLeaseNode  *string    // migrations/00565
}
...
type JobStore interface {
	...
	// OCI entrypoint (migrations/00563, capped at 64 entries).
	...
	// PL/pgSQL helper (migrations/00567) on PgStore; memstore mirrors
