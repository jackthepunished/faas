...
func TestMemStoreCoverageAuthUsageAndEvents(t *testing.T) {
	...
	if err := m.AppendUsage(ctx, account.ID, app.ID, "instance", minute, 100, 2, 0, 0, 0, 0, 0); err != nil {
		t.Fatal(err)
	}
	if err := m.AppendUsage(ctx, account.ID, app.ID, "instance", minute.Add(30*time.Second), 999, 999, 0, 0, 0, 0, 0); err != nil {
		t.Fatal(err)
	}
