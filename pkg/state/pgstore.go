...
// AppendDeploymentStage (ADR-117, migration 00296) atomically
...
// (migrations/00296_deployments_stage_state.sql) so a typo from a
...
func (s *PgStore) AppendDeploymentStage(ctx context.Context, id string, from, to StageName, at time.Time, reason string) (Deployment, error) {
	...
	if state.Current != from {
		...
		// "source_download" (migrations/00296). A pre-existing row
