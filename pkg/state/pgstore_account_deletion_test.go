...
func TestPg_UsageByAccount_AggregatesByMonth(t *testing.T) {
	...
	if err := s.AppendUsage(ctx, acctID, app.ID, ins.ID, minute, 1024, 5, 0, 0, 0, 0, 0, 0); err != nil {
		t.Fatalf("AppendUsage: %v", err)
	}
	if err := s.AppendUsage(ctx, acctID, app.ID, ins.ID, minute.Add(time.Minute), 2048, 7, 0, 0, 0, 0, 0, 0); err != nil {
		t.Fatalf("AppendUsage: %v", err)
	}
