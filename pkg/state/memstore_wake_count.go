...
// stub. Postgres-only. Returns the zero value so a unit test that
// inadvertently exercises the per-app dashboard's wakes_24h field
// against MemStore surfaces the sentinel error rather than silently
// returning 0 (a 0 result is indistinguishable from a fresh app
// with no traffic, and a wrong-zero would mask the test bug). The
// handler is best-effort: 0 + nil error degrades to "no wakes
// recorded today"; 0 + sentinel error surfaces as the existing
// `s.log.Warn("wakes_24h fetch failed", ...)` line in
// cmd/apid/handlers_metrics.go's enrichment loop.
