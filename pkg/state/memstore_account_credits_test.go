...
func TestMemStoreCurrentMonthOverageCents_Formula(t *testing.T) {
	...
	now := time.Now().UTC()
	if err := s.AppendUsage(ctx, acct.ID, "app-1", "inst-1", now, mbSeconds, 0, 0, 0, 0, 0, 0); err != nil {
...
func TestMemStoreCurrentMonthOverageCents_BeforeMonthStart(t *testing.T) {
	...
	if err := s.AppendUsage(ctx, acct.ID, "app-1", "inst-1", prevMonth, 3_600_000, 0, 0, 0, 0, 0, 0); err != nil {
		t.Fatalf("append usage: %v", err)
	}
